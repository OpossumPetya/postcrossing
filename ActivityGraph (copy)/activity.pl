...
get '/' => sub {
    my $self = shift;
    return $self->redirect_to('/u/paulo');
};
...
get '/u/:username' => sub {
    ...
    my $agent = WWW::Mechanize->new(
        agent => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.90 Safari/537.36', # Chrome 77 on Win10 x64
        cookie_jar => $cookie_jar,
        autocheck => 0, # don't die on errors
    );
    ...
    if ($response->header('Content-Type') !~ /json/) {
        ...
        $agent->get('https://www.postcrossing.com/login');
